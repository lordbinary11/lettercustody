...
export function IncomingLetterActions({ letterId, movementId, onSuccess }: IncomingLetterActionsProps) {
  ...
  async function handleReceive() {
    ...
    if (result.success) {
      showToast('Letter accepted successfully', 'success');
      onSuccess?.();
      router.refresh();
    } else {
      const errorMsg = result.error || 'Failed to receive letter';
      setError(errorMsg);
  ...
  async function handleReject() {
    ...
    if (result.success) {
      showToast('Letter rejected successfully', 'success');
      setShowReject(false);
      setRejectionReason('');
      onSuccess?.();
      router.refresh();
    } else {
      const errorMsg = result.error || 'Failed to reject letter';
      setError(errorMsg);
      showToast(errorMsg, 'error');
    }
  }
